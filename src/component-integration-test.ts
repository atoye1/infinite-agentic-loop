...
export class ComponentIntegrationTest {
  ...
  private async testDataProcessing(): Promise<void> {
    try {
      ...
      if (!firstItem.id || !firstItem.name || typeof firstItem.value !== 'number') {
        this.errors.push('Data item missing required properties (id, name, value)');
        return;
      }
  ...
  private async testUtilityFunctions(): Promise<void> {
    try {
      ...
      if (!dimensions.width || !dimensions.height || dimensions.width <= 0 || dimensions.height <= 0) {
        this.errors.push('calculateContainerDimensions() returned invalid dimensions');
      }
  ...
  private async testPropsValidation(): Promise<void> {
    try {
      ...
      const { barChartRaceSchema } = await import('./BarChartRaceComposition');
      ...
      if (!validation.success) {
        this.errors.push(`Props validation failed: ${validation.error.message}`);
      }
  ...
  private async testComponentInstantiation(): Promise<void> {
    try {
      ...
      if (typeof BarChartRaceComposition !== 'function') {
        this.errors.push('BarChartRaceComposition is not a valid React component');
      }
